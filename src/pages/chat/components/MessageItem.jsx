    ...
    <>
      <div className="flex gap-2 items-start">
        <DeleteDialog
          ...
          onOpenChange={() =>
            __dispatch_local({ type: "CLOSE_DELETE_DIALOG" })
          }
        ...
        {avatar_url ? (
          ...
          <UserFallback name={full_name} _idx={message.id[0]} />
        )}
        ...
        <div className="relative message-body w-full text-(--muted-foreground) group">
          ...
          >
            {contact && (
              <div className="bg-(--background) absolute -top-10 w-40 h-20 text-(--foreground)">
                
                <button
                  onClick={() =>
                    handleFunction(message.profiles)
                  }
                  className="bg-gray-600 py-2 px-2"
                >
                  message
                </button>
              </div>
